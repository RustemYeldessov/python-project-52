...
class TestLabelCRUD:
    ...
    # ---------- CREATE ----------
    def test_create_label_get(self, logged_client):
        response = logged_client.get(reverse('label_create'))
        assert response.status_code == 200
    ...
    # ---------- UPDATE ----------
    def test_update_label_get(self, logged_client, label):
        response = logged_client.get(reverse('label_update', args=[label.id]))
        assert response.status_code == 200
    ...
    # ---------- DELETE ----------
    def test_delete_label_get(self, logged_client, label):
        response = logged_client.get(reverse('label_delete', args=[label.id]))
        assert response.status_code == 200
        assert "Удалить метку" in response.content.decode()
